...
import { createUserSchema, updateUserSchema, updateUserRoleSchema, demoteUserRoleSchema, hardDeleteUserSchema, uuidParamSchema } from '../validation/schemas.js';
...
router.patch('/:id/role/demote', validateParams(uuidParamSchema), validate(demoteUserRoleSchema), async (req: Request, res: Response): Promise<void> => {
  try {
    const id = getParam(req, 'id');
    ...
    if (user.role !== 'employee') {
      res.status(400).json({ error: 'Only employee users can be demoted', code: 'INVALID_ROLE_CHANGE' });
      return;
    }

    const updated = await usersRepo.updateRole(id, 'client');
    if (!updated) {
      res.status(404).json({ error: 'User not found', code: 'NOT_FOUND' });
      return;
    }
...
router.delete('/:id/hard', validateParams(uuidParamSchema), validate(hardDeleteUserSchema), async (req: Request, res: Response): Promise<void> => {
  try {
    const id = getParam(req, 'id');
    ...
    if (actor.id === user.id) {
      res.status(403).json({ error: 'Cannot hard delete your own account', code: 'FORBIDDEN' });
      return;
    }

    const deleted = await usersRepo.hardDelete(id);
    if (!deleted) {
      res.status(404).json({ error: 'User not found', code: 'NOT_FOUND' });
      return;
    }
